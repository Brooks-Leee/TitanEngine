...
#endif
	//1M_Cube.dat
	const char* FilePath = "Assets\\StaticMesh\\SM_TableRound.dat";
...
void BoxApp::GetLoadedBinaryFile(FStaticMeshInfo* LoadedStruct)
{
...
{
	std::vector<uint32_t> indices = mLoadedStruct->indices;
	indices.resize(mLoadedStruct->indices.size());

	size_t verticesLen = mLoadedStruct->Vertices.size();
	std::vector<Vertex> vertices;
	vertices.resize(verticesLen);
	for (int i = 0; i < verticesLen; i++)
	{
		vertices[i].Pos.x = mLoadedStruct->Vertices[i].x;
		vertices[i].Pos.y = mLoadedStruct->Vertices[i].y;
		vertices[i].Pos.z = mLoadedStruct->Vertices[i].z;
		if (i % 2 == 0)
		{
